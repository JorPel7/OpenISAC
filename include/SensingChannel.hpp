...
public:
    ...
    // ZMQ monostatic mode: start sensing loop without USRP RX thread.
    // Feed frames via push_zmq_rx_frame() from an external ZMQ receive thread.
    void start_zmq_rx();
    bool push_zmq_rx_frame(AlignedVector&& samples, uint64_t frame_seq);

    // Acquire/release RX frame buffers from the internal pool.
    // ZMQ callers MUST use these instead of allocating new vectors so the pool
    // stays bounded and frames can be reused without heap pressure.
    AlignedVector acquire_rx_frame();
    void release_rx_frame(AlignedVector&& buf);
