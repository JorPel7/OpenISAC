...
#ifdef OPENISAC_ZMQ_SUPPORT
        if (_cfg.zmq_mode && !_sensing_channels.empty() && _zmq_sensing_pull) {
            _zmq_sensing_rx_thread = std::thread(&OFDMISACEngine::_zmq_sensing_rx_proc, this);
        }
#endif
    }
...
#ifdef OPENISAC_ZMQ_SUPPORT
        if (_zmq_sensing_rx_thread.joinable()) _zmq_sensing_rx_thread.join();
#endif
...
private:
    ...
    std::unique_ptr<zmq::socket_t> _zmq_push;
    std::unique_ptr<zmq::socket_t> _zmq_sensing_pull;
    std::thread _zmq_sensing_rx_thread;
    std::atomic<uint64_t> _zmq_rx_frame_seq{0};
#endif
    ...
    void _init_usrp() {
        ...
        if (_cfg.zmq_mode) {
            ...
            _zmq_push->set(zmq::sockopt::sndtimeo, 500);  // 500ms — don't hang if ChannelSim exits
            _zmq_push->connect("tcp://127.0.0.1:5558");
            LOG_G_INFO() << "[ZMQ] TX connecting to tcp://127.0.0.1:5558";
            if (!_sensing_channels.empty()) {
                ...
                LOG_G_INFO() << "[ZMQ] Sensing RX connecting to tcp://127.0.0.1:5559";
            }
            return;
        }
#endif
...
#ifdef OPENISAC_ZMQ_SUPPORT
        if (_cfg.zmq_mode) {
            zmq::message_t msg(total_samps * sizeof(std::complex<float>));
            std::memcpy(msg.data(), data, msg.size());
            try {
                const bool ok = _zmq_push->send(msg, zmq::send_flags::none).has_value();
                return ok ? total_samps : 0;
            } catch (const zmq::error_t&) {
                return 0;   // ChannelSimulator disconnected — caller handles as underflow
            }
        }
#endif
    ...
    void _zmq_sensing_rx_proc() {
        ...
        while (_running.load(std::memory_order_relaxed)) {
            ...
            try {
                ...
                if (e.num() == ETERM) break;   // context destroyed — clean exit
                LOG_RT_WARN_HZ(2) << "[ZMQ] sensing recv error: " << e.what();
                continue;
            }
            ...
            while (src_pos < msg_samps) {
                const size_t needed = frame_size - frame_received;
                const size_t to_copy = std::min(needed, msg_samps - src_pos);
                ...
                if (frame_received >= frame_size) {
                    const uint64_t seq = _zmq_rx_frame_seq.fetch_add(1, std::memory_order_relaxed);
                    for (auto& ch : _sensing_channels) {
                        // Acquire from pool to avoid unbounded heap growth:
                        // the sensing loop releases frames back to the pool, so
                        // we must also source them from the pool.  Allocating a
                        // fresh vector each time fills the pool without draining
                        // it, causing a ~14 GB leak after ~37 s at 434 Hz.
                        AlignedVector ch_buf = ch->acquire_rx_frame();
                        if (ch_buf.size() != frame_size) {
                            ch_buf.resize(frame_size);
                        }
                        std::copy(frame_buf.begin(), frame_buf.end(), ch_buf.begin());
                        ch->push_zmq_rx_frame(std::move(ch_buf), seq);
                        // On push failure, push_zmq_rx_frame releases ch_buf back
                        // to the pool automatically, so no extra release needed here.
                    }
                    frame_received = 0;
                }
            }
...
int UHD_SAFE_MAIN(int argc, char* argv[]) {
    ...
    std::signal(SIGTERM, &signal_handler);   // handle kill/systemd gracefully
    ...
    if (zmq_mode) {
        LOG_G_INFO() << "[ZMQ] Monostatic ZMQ mode: TX→ChannelSim→sensing RX, output port "
                     << cfg.mono_sensing_port;
        // ZMQ pipeline has ~10-30 frame latency; ensure the queue is large enough.
        // Use the YAML value (paired_frame_queue_size) but enforce a minimum of 256
        // so that transient processing spikes don't cause TX-frame drops.
