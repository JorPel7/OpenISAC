...
private:
...
#ifdef OPENISAC_ZMQ_SUPPORT
    std::unique_ptr<zmq::context_t> _zmq_ctx;
    std::unique_ptr<zmq::socket_t> _zmq_pull;
    std::vector<std::complex<float>> _zmq_staging;
    size_t _zmq_staging_pos = 0;
#endif
    ...
    uhd::time_spec_t _hw_time_now() const {
        return usrp_ ? usrp_->get_time_now() : uhd::time_spec_t(0.0);
    }

#ifdef OPENISAC_ZMQ_SUPPORT
    size_t zmq_recv_samps(std::complex<float>* buf, size_t max_samps) {
        // Serve from staging buffer first
        const size_t avail = _zmq_staging.size() - _zmq_staging_pos;
        if (avail > 0) {
            const size_t n = std::min(avail, max_samps);
            std::memcpy(buf, _zmq_staging.data() + _zmq_staging_pos,
                        n * sizeof(std::complex<float>));
            _zmq_staging_pos += n;
            if (_zmq_staging_pos >= _zmq_staging.size()) {
                _zmq_staging.clear();
                _zmq_staging_pos = 0;
            }
            return n;
        }
        // Pull a new ZMQ message
        zmq::message_t msg;
        const auto res = _zmq_pull->recv(msg, zmq::recv_flags::none);
        if (!res || msg.size() == 0) return 0;  // timeout
        const size_t msg_samps = msg.size() / sizeof(std::complex<float>);
        const size_t to_copy   = std::min(msg_samps, max_samps);
        std::memcpy(buf, msg.data(), to_copy * sizeof(std::complex<float>));
        if (to_copy < msg_samps) {
            const auto* src = static_cast<const std::complex<float>*>(msg.data());
            _zmq_staging.assign(src + to_copy, src + msg_samps);
            _zmq_staging_pos = 0;
        }
        return to_copy;
    }
    ...
    void init_usrp() {
        ...
        if (cfg_.zmq_mode) {
            ...
            _zmq_pull = std::make_unique<zmq::socket_t>(*_zmq_ctx, zmq::socket_type::pull);
            _zmq_pull->set(zmq::sockopt::rcvtimeo, 100);
            const std::string addr = "tcp://127.0.0.1:5559";
            _zmq_pull->connect(addr);
            LOG_G_INFO() << "[ZMQ] RX connecting to " << addr;
            _rx_gain_min_db = 0.0;
            _rx_gain_max_db = 76.0;
            _rx_agc.initialize(cfg_.rx_gain, _rx_gain_min_db, _rx_gain_max_db);
            _sync_search_gain_sweep.initialize(cfg_.rx_gain, _rx_gain_min_db, _rx_gain_max_db);
            return;
        }
#endif
    ...
    void adjust_rx_freq(double detected_offset, bool reset = false) {
        ...
        if (usrp_) current_rx_tune_ = usrp_->set_rx_freq(new_tune_req, cfg_.rx_channel);
    ...
    void _enter_sync_search_state() {
        ...
        const bool reset_search_gain = _sync_search_gain_sweep.reset_to_default(
            [this](double gain_db) {
                if (usrp_) usrp_->set_rx_gain(gain_db, cfg_.rx_channel);
            },
            [this](double gain_db) {
...
#ifdef OPENISAC_ZMQ_SUPPORT
            if (cfg_.zmq_mode) {
                got = zmq_recv_samps(&target_buffer[received],
                                     cfg_.sync_samples() - received);
            } else
#endif
            {
                got = rx_stream_->recv(&target_buffer[received],
                                       cfg_.sync_samples() - received, md);
            }
            if (received == 0 && got > 0) {
...
#ifdef OPENISAC_ZMQ_SUPPORT
            if (cfg_.zmq_mode) {
                got = zmq_recv_samps(&temp_buf[received], total_read - received);
            } else
#endif
            {
                got = rx_stream_->recv(&temp_buf[received], total_read - received, md);
            }
            if (received == 0 && got > 0) {
...
#ifdef OPENISAC_ZMQ_SUPPORT
            if (cfg_.zmq_mode) {
                got = zmq_recv_samps(&frame.frame_data[received],
                                     cfg_.samples_per_frame() - received);
            } else
#endif
            {
                got = rx_stream_->recv(&frame.frame_data[received],
                                       cfg_.samples_per_frame() - received, md);
            }
            if (received == 0 && got > 0) {
    ...
    void process_sync_data(const AlignedVector& sync_data, int64_t sync_time_ns) {
        ...
        if (max_corr/avg_corr > energy_threshold) {
            ...
            if (issued_freq_adjust) {
                _control_time_gates.mark_freq_adjust_now(_hw_time_now());
            }
            if (issued_alignment) {
                _control_time_gates.mark_alignment_now(_hw_time_now());
            }
        } else {
    ...
    void process_ofdm_frame(const RxFrame& frame, int64_t frame_dequeue_time_ns) {
        ...
        if((max_mag/average_mag < 20.0f || (abs(delay_index_err) > cfg_.delay_adjust_step+5)) &&
            ...
            if (_reset_count >= _reset_hold_frames) {
                ...
                _control_time_gates.mark_reset_now(_hw_time_now());
                return;
            }
        }
        else
        ...
        if (issued_rx_gain_adjust) {
            _control_time_gates.mark_rx_gain_adjust_now(_hw_time_now());
        }
...
int UHD_SAFE_MAIN(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--zmq") {
            ...
            LOG_G_ERROR() << "Unknown CLI argument: " << arg
                          << ". Supported flags: --zmq";
            return 1;
        }
    }
#ifndef OPENISAC_ZMQ_SUPPORT
    if (zmq_mode) {
        LOG_G_ERROR() << "--zmq requested but ZMQ support was not compiled in.";
        return 1;
    }
#endif
    ...
    if (zmq_mode) {
        LOG_G_INFO() << "[ZMQ] Receiving I/Q from ZMQ channel simulator on tcp://127.0.0.1:5559";
        LOG_G_INFO() << "[ZMQ] Bistatic sensing output on port " << cfg.bi_sensing_port;
    }
