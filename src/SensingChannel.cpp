...
public:
    ...
    // Enable ZMQ loopback mode: _pop_next_tx uses a timeout instead of
    // blocking forever.  This prevents the sensing loop from hanging when
    // TX sensing frames are dropped because the paired TX queue was full.
    void set_zmq_loopback(bool v) { _zmq_loopback.store(v, std::memory_order_relaxed); }
    ...
    bool pop_pair(AlignedVector& rx_out, TxSymbolsFrame& tx_out) {
        ...
        while (true) {
            ...
            if (!have_tx) {
                if (_zmq_loopback.load(std::memory_order_relaxed)) {
                    // ZMQ loopback mode: try for up to 200 ms, then discard
                    // the current RX frame and resync.  This breaks the
                    // infinite-block that occurs when TX sensing frames are
                    // dropped (paired TX queue full) and their seq numbers
                    // are permanently ahead of what the sensing loop expects.
                    bool timed_out = false;
                    if (!_pop_next_tx_zmq(tx_item, timed_out)) {
                        _recycle_one(std::move(rx_item.samples));
                        if (timed_out) {
                            // TX dropped: give up on this RX frame and restart
                            have_rx = false;
                            continue;
                        }
...
private:
    ...
    // ZMQ loopback variant: returns false+timed_out=true after 200 ms so the
    // sensing loop can discard the orphaned RX frame and resync, instead of
    ...
    bool _pop_next_tx_zmq(TxSymbolsFrame& out, bool& timed_out) {
        ...
        while (Clock::now() < deadline) {
            ...
            if (_closed.load(std::memory_order_relaxed)) {
                return false;   // closed — caller propagates exit
            }
            backoff.pause();
        }
        timed_out = true;
        return false;   // timeout — caller discards RX frame and resyncs
    }
    ...
    std::atomic<bool> _zmq_loopback{false};  // set by set_zmq_loopback()
...
void SensingChannel::start_zmq_rx() {
    ...
    _rx_io.next_rx_frame_seq = 0;
    // Enable ZMQ loopback mode: pop_pair() uses a timeout on the TX side so
    // the sensing loop never blocks forever when TX sensing frames are dropped.
    _rx_io.paired_queue->set_zmq_loopback(true);
    _compute.sensing_sender.start();
    // No _rx_io.rx_thread — the ZMQ thread in OFDMISACEngine feeds frames via push_zmq_rx_frame()
    _compute.sensing_thread = std::thread(&SensingChannel::_sensing_loop, this);
}
...
bool SensingChannel::push_zmq_rx_frame(AlignedVector&& samples, uint64_t frame_seq) {
    ...
    if (!_rx_io.paired_queue->push_rx(std::move(rx_item))) {
        ...
        // SPSCRingBuffer::try_push does NOT move the value on failure, so
        // rx_item.samples is still valid here.  Return it to the pool so the
        // ZMQ caller's pool-acquire / release cycle stays balanced.
